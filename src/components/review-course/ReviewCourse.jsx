...
const ReviewCourse = (props) => {
  ...
  const reviewsShow = listReview
    ?.sort((a, b) => b.id - a.id)
    .slice(0, listSize);

  const updateList = () => {
    listSize + defaultValueIncreased < listReview?.length
      ? setListSize(listSize + defaultValueIncreased)
      : setListSize(listReview.length);
  };
  ...
  const fiveStar =
    countReview > 0
      ? Math.round(
          (listReview.filter((x) => x.rate === 5).length / countReview) * 100
        )
      : 0;
  const fourStar =
    countReview > 0
      ? Math.round(
          (listReview.filter((x) => x.rate === 4).length / countReview) * 100
        )
      : 0;
  const threeStar =
    countReview > 0
      ? Math.round(
          (listReview.filter((x) => x.rate === 3).length / countReview) * 100
        )
      : 0;
  const twoStar =
    countReview > 0
      ? Math.round(
          (listReview.filter((x) => x.rate === 2).length / countReview) * 100
        )
      : 0;
  const oneStar =
    countReview > 0
      ? Math.round(
          (listReview.filter((x) => x.rate === 1).length / countReview) * 100
        )
      : 0;
  ...
  useEffect(() => {
    ...
    setAverageRate(
      Math.round(
        (listReview?.reduce((a, c) => a + c.rate, 0) / countReview) * 10
      ) / 10
    );
    return () => {
      return [];
  ...
  return (
    <div className="">
      ...
      <div className="loadmore-container">
        ...
        ) : listSize >= defaultSize && defaultSize < listReview.length ? (
          <div
