...
const LearningTool = (props) => {
  ...
  const handleSubmit = (values) => {
    ...
    const data = radioValue === "email" ?
      { email: values.email, body: values.content, subject: "Things I wanna say to my Future self", dateTime: values.time, timeZone: "Asia/Ho_Chi_Minh" }
      :
      { phone: values.phone, message: values.content, dateTime: values.time, timeZone: "Asia/Ho_Chi_Minh" }
  ...
  return (
    <div className='learning-tool-container'>
      ...
      <div className={'show-' + !isCreateEvent}>
        <button className='learning-tool-schedule-btn' onClick={() => setIsCreateEvent(!isCreateEvent)}>
          <span style={{ "marginRight": "10px" }}>Schedule learning time</span> <i class='bx bx-sm bx-alarm-add'></i>
        </button>
      </div>
      <div className={'learning-tool-schedule-content show-' + isCreateEvent}>
        ...
        >
          {(formik) => (
            <div>
              <h2 className="my-4 font-weight-bold .display-4">Create an event</h2>
              <Form>
                ...
                <FormControl>
                  <FormLabel id="demo-radio-buttons-group-label">
                    <div style={{ "fontSize": "1rem", "fontWeight": "700", "marginTop": "10px" }}>
                      Notifice By
                    </div>
                  </FormLabel>
                  <RadioGroup
