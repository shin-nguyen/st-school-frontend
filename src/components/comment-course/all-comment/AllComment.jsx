...
import { fetchCommentById } from "../../../services/comment-course-service";
import { useDispatch, useSelector } from "react-redux";
import defaultAvatar from "../../../assets/images/kai.jpg"

const AllComment = (props) => {
    const dispatch = useDispatch();
    const comments = useSelector(state => state.commentCourse.comments);
    const commentsShow = comments.sort((a, b) => b.id - a.id);
    const { course } = props
    console.log(course)

    useEffect(() => {
        dispatch(fetchCommentById(course.id));
    }, [course]);
                    ...
                    <div className="comment" span={18} style={{ marginTop: "1rem" }} xs={24} sm={24} md={18}>
                        <div className="all-comment-info">
                            <div style={{ display: "flex" }}>
                                ...
                                {
                                    // comment.user.id === course.user.id ? (
                                    //     <strong>
